                ...
                for (notCached : action.notCached) {
                    try {
                        ...
                        val imageStream = Platform.getBundle(bundle)
                            .getResource(path)
                            .openStream
                        // This would work if we just use a java version > java-8
//                        val imageBytes = imageStream.readAllBytes
                        val imageBytes = readAllBytes(imageStream)
                        imageStream.close
    ...
    // Copied from newer java.io.InputStream code to make it function like java-9 or higher.
    public def readAllBytes(InputStream i) {
        var len = Integer.MAX_VALUE;
        var List<byte[]> bufs = null;
        var byte[] result = null;
        var int total = 0;
        var int remaining = len;
        var int n;
        do {
            var byte[] buf = newByteArrayOfSize(Math.min(remaining, 8192));
            var int nread = 0;

            // read to EOF which may read more or less than buffer size
            while ((n = i.read(buf, nread,
        ...
        result = newByteArrayOfSize(total);
        var int offset = 0;
        remaining = total;
        for (byte[] b : bufs) {
            var int count = Math.min(b.length, remaining);
