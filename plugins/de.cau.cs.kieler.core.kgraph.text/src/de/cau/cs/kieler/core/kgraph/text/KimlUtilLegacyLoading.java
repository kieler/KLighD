...
/**
 * @deprecated Only present for the KGraph<->KGraph interoperability.
 */
public class KimlUtilLegacyLoading {
    ...
    /**
     * A predicate returning true if the passed element is an instance of {@link IPropertyHolder}.
     */
    ...
    public static KNode loadDataElements(final KNode graph,
        ...
        /*
         * This is basically the same as graph.eAllContents(). However, using the latter would cause
         * a ConcurrentModificationException. The reason we're walking through the graph here is
         * that we rebuild properties based on persistent entries. But with eAllContents(), we would
         * also iterate over the properties we're modifying. That's where the exception comes in. To
         * avoid that, we're using a special tree iterator that skips over properties of
         * EMapPropertyHolders. Magic! (KIPRA-1541)
         */
        ...
        if (id != null && value != null) {
            ...
            if (layoutOptionData != null) {
                ...
                if (knownProps.containsKey(id)) {
                    ...
                    // FIXME ELK-MIGRATION LayoutOptionProxy is internal API
    ...
    private static class PropertiesSkippingTreeIterator extends AbstractTreeIterator<EObject> {
        ...
        protected Iterator<? extends EObject> getChildren(final Object object) {
            ...
            // The iterator will usually be a FeatureIteratorImpl that we can set a feature filter
            // on
                    ...
                    public boolean isIncluded(final EStructuralFeature eStructuralFeature) {
                        ...
                        if (eStructuralFeature.getContainerClass()
                            ...
                            return eStructuralFeature
                                    .getFeatureID() != KGraphPackage.EMAP_PROPERTY_HOLDER__PROPERTIES;
                        } else {
                            return true;
                        }
                    }
