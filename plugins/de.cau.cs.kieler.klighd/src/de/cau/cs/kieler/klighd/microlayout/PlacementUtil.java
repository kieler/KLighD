...
public final class PlacementUtil {
    ...
    public static Bounds estimateTextSize(final KText kText, final String text) {
        ...
        return PlacementUtil.estimateTextSizeMock(PlacementUtil.fontFor(kText, null), text);
    }
...
//            }
    ...
    public static Font fontFor(final ElkLabel label) {
        return PlacementUtil.fontFor(ktextFor(label), null);
    }
    ...
    /**
     * Estimate text using a mock value to avoid an SWT call and a call of {@code BufferedImage.createGraphics} which
     * both create deadlocks for Mac in an LS setting.
     * 
     * @param font The font
     * @param text The text
     * @return A mock value 
     */
    public static Bounds estimateTextSizeMock(final Font font, final String text) {
        return new Bounds(Strings.isNullOrEmpty(text) ? 0 : text.length() * font.getSize(), font.getSize());
    }

    /**
     * Find the String with the biggest width according to a font in string array.
     * Uses a mock width to estimate the text size.
     * 
     * @param font
    ...
    public static float getWidthOfBiggestWord(final Font font, final String[] words) {
        Bounds bWSize = PlacementUtil.estimateTextSizeMock(font, "");
        Bounds textPartSize;

        for (String textpart : words) {
            textPartSize = PlacementUtil.estimateTextSizeMock(font, textpart);
    ...
    public static String findFittingString(final String text, final Font font,
        ...
        Bounds newSize = PlacementUtil.estimateTextSizeMock(font, textWithoutWraps);
        String newText = "";
        ...
        newSize = PlacementUtil.estimateTextSizeMock(font, newText);
        ...
        if (newSize.getWidth() > targetWidth) {
            ...
            while (newSize.getWidth() < targetWidth
                ...
                newSize = PlacementUtil.estimateTextSizeMock(font, newTextCandidate);
